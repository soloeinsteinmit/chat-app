 ...
 * @returns {Object} An object containing the list of friends with their associated chat, and any error that occurred during fetching.
 */
        ...
        const friendsData = await Promise.all(
          userChats.map(async (chat) => {
            ...
            return {
              ...
              chat, // Include the original chat object
            };
          })
        );

        // Filter out any `null` values that may have been returned
        setFriends(friendsData.filter((item) => item !== null));
      } catch (error) {
